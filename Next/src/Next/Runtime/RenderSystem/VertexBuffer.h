...
namespace Next {
	...
	struct VertexBufferElement
	{
		...
		VertexBufferElement(ShaderDataType type, const std::string& name, bool normalized = false)
			: Name(name), Type(type), Size(ShaderDataTypeToSize(type)), Offset(0), Normalized(normalized)
		{
		}
...
namespace std {
	...
	template<> struct hash<Next::Vertex> {
		size_t operator()(Next::Vertex const& vertex) const {
