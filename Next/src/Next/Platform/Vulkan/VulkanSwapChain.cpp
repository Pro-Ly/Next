...
namespace Next {
	...
	void VulkanSwapChain::Init(VkInstance vkInstance, Ref<VulkanDevice>& device)
	{
		m_Device = device;
	}
	...
	void VulkanSwapChain::Create(GLFWwindow* window)
	{
		...
		//wait on the driver to complete internal operations before we can acquire another
		//image to render to. Therefore it is recommended to request at least one more image than the minimum:
		uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1;

		// not exceed the maximum number of images while doing this
		...
		QueueFamilyIndices indices = VulkanPhysicalDevice::findQueueFamilies(phycicalDevice);
		...
		if (vkCreateSwapchainKHR(m_Device->GetLogicalDevice(), &createInfo, nullptr, &m_SwapChain) != VK_SUCCESS) {
			NX_CORE_ASSERT(false, "failed to create swap chain!");
		}
